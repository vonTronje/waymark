...
class ShareableTest < ActiveSupport::TestCase
  ...
  test "accessible_to scope filters by share level" do
    shared = ShareableTestDocument.create!(title: "Shared")
    other = ShareableTestDocument.create!(title: "other")
    shared.shares.create!(user: users(:player_two), access: :viewer)

    assert_empty ShareableTestDocument.accessible_to(users(:player_two), as: :editor)
    assert_equal [ shared ], ShareableTestDocument.accessible_to(users(:player_two), as: :viewer).to_a
    assert_equal [ shared, other ], ShareableTestDocument.accessible_to(users(:admin)).order(:id)
  end
